...
export function GuestResults() {
  ...
  const handleDownload = () => {
    const blob = new Blob([JSON.stringify({ extracted_data: data, gap_analysis: gaps, benchmark }, null, 2)], {
      type: "application/json",
    });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "brsr_extracted_data.json";
    a.click();
          ...
          {gaps && gaps.missing_mandatory.length > 0 && (
            <div className="mt-10 mb-8">
              ...
              {gaps.missing_core.length > 0 && (
                <div className="bg-white rounded-2xl border-2 border-red-200 overflow-hidden mb-6">
              ...
              {(() => {
                ...
                return (
                  <div className="bg-white rounded-2xl border-2 border-amber-200 overflow-hidden mb-6">
                    <div className="px-6 py-4" style={{ background: "linear-gradient(135deg, #FFFBEB 0%, #FEF3C7 100%)" }}>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-amber-600 mt-0.5">Required for complete BRSR filing — address after Core gaps</p>
                        </div>
